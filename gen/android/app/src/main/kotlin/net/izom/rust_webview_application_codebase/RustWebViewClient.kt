...
class RustWebViewClient(context: Context) : WebViewClient() {
    ...
    override fun onReceivedError(
        ...
        // so we must force retry here with loadUrl() to get a chance of the custom protocol to kick
        // in
        if (
            error.errorCode == ERROR_CONNECT &&
                request.isForMainFrame &&
                request.url != lastInterceptedUrl
        ) {
            // prevent the default error page from showing
            view.stopLoading()
            // without this initial loadUrl the app is stuck
            view.loadUrl(request.url.toString())
            // ensure the URL is actually loaded - for some reason there's a race condition and we
            // need to call loadUrl() again later
